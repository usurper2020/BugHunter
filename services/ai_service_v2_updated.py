...
class AIService:
    ...
    def __init__(self):
        ...
        self.api_key = config.get('AI_API_KEY', '')  # Get API key from config.json
        ...
        if self.api_type == 'openai':
            # Use environment variable if config key is not available
            if not self.api_key:
                self.api_key = os.getenv('OPENAI_API_KEY', '')
                if not self.api_key:
                    raise ValueError("OpenAI API key not found in config.json or environment variables. Please set 'AI_API_KEY' in your configuration or 'OPENAI_API_KEY' environment variable.")
            logger.debug(f"Using OpenAI API key: {self.api_key}")  # Debug log for API key
    ...
    async def analyze_vulnerability(self, finding: Finding) -> Dict[str, Any]:
        ...
        logger.debug(f"Analyzing vulnerability: {finding.id}")  # Debug log for vulnerability analysis
        ...
        try:
            ...
            logger.debug(f"Analysis result: {analysis}")  # Log the analysis result
            return analysis
